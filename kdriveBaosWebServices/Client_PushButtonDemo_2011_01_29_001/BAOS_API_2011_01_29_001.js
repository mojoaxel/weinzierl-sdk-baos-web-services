...
function BaosLib()
{
	...
	this.API_GetServerItem = function (strStartItem, strItemCount)
	{
		   ...
		   (IsNumber(strStartItem)) && (IsNumber(strItemCount)))				// If we have a valid IP address and valid parameters
		{
			strUrl = "http://" + m_strIpAddr +
					 ...
					 "&ItemCount=" + strItemCount + "&Callback=?";				// Prepare URL for GetServerItem
			
			$.jsonp(															// Send URL to BAOS and wait for JSON response
			{
				"url": strUrl,													// URL
				"success": OnObjSrvRespRcvd,									// Reference to callback function (response)
				"error": OnObjSrvError											// Reference to callback function (error)
			});
		}
	...
	this.API_GetDatapointDescription = function (strDatapointStart, strDatapointCount)
	{
		   ...
		   (IsNumber(strDatapointStart)) && (IsNumber(strDatapointCount)))		// If we have a valid IP address and valid parameters
		{
			strUrl = "http://" + m_strIpAddr +
					 ...
					 strDatapointCount + "&Callback=?";							// Prepare URL for GetDatapointDescription
	...
	this.API_GetDescriptionString = function (strDatapointStart, strDatapointCount)
	{
		   ...
		   (IsNumber(strDatapointStart)) && (IsNumber(strDatapointCount)))		// If we have a valid IP address and valid parameters
		{
			strUrl = "http://" + m_strIpAddr +
					 ...
					 strDatapointStart + "&DatapointCount=" +
					 strDatapointCount + "&Callback=?";							// Prepare URL for GetDescriptionString
	...
	this.API_GetDatapointValue = function (strDatapointStart, strDatapointCount, strFormat, strFilter)
	{
		   ...
		   (IsNumber(strDatapointStart)) && (IsNumber(strDatapointCount)))		// If we have a valid IP address and valid parameters
		{
			strUrl = "http://" + m_strIpAddr +
					 ...
					 strDatapointStart + "&DatapointCount=" +
					 strDatapointCount + "&Format=";							// Prepare URL for GetDatapointValule

			strUrl += strFormat;												// Add format
			
			strUrl += "&Filter=";												// Add filter parameter
			strUrl += strFilter;												// Add filter parameter
			
			strUrl += "&Callback=?";											// Add callback parameter
	...
	this.API_StartIndicationListener = function (strFormat)
	{
		...
		if((IsIpAddrValid(m_strIpAddr)))										// If we have a valid IP address
		{
			...
			$.jsonp(
			{
				...
				{
					if((data.Result == true) &&
					   (data.Service == "StartIndicationSession"))				// If indication session could be started
					{
						m_IndicationSession = data.Data.SessionId;				// Save indication session ID
	...
	this.API_StopIndicationListener = function ()
	{
		...
		if((IsIpAddrValid(m_strIpAddr)))										// If we have a valid IP address
		{
			...
			$.jsonp(
			{
				...
				{
					if((data.Result == true) &&
					   (data.Service == "StopIndicationSession"))				// If indication session could be stopped
					{
	...
	function IsIpAddrValid(strIpAddrToValidate)
	{
		...
		for (var nIndex = 0; nIndex <strIpAddrSub.length; nIndex++)				// Loop to handle every IP address sub part
		{
	...
	{
		for(var position=0; position<strToValidate.length; position++)			// Loop to handle every position in string
		{
		...
		if((IsIpAddrValid(m_strIpAddr)))										// If we have a valid IP address
		{
			...
			$.jsonp(															// Send URL to BAOS and wait for JSON response
			{
				...
				"success": function(data)
				{
					...
					if(data.Result == false)									// If we got an error
					{
						if(data.Error == "IndTimeout")							// If we have a timeout
						{
					...
					if(data.Service == "GetIndication")							// If we have a GetIndication response
					{
						if(data.Result == true)									// If we got an indication
						{
							OnIndicationUpdate(data);							// Indication update
	...
	function OnObjSrvError()
	{
		...
		{
			m_CallbackTransmitError();											// Call callback function
		}
	}
}
